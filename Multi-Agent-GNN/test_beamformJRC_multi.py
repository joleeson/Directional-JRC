...
class Agent(object):
    def __init__(self, policy_config, sample_trajectory_args, env_args):
        ...
        self.radar_interval = policy_config['radar_interval']
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        acs, rewards, entropys, terminals = [], [], [], []
        steps = 0
        ac = np.zeros((self.num_users,), dtype=int)
        prev_comm_ac = np.zeros((self.num_users,), dtype=int)
        
        for i in range(self.num_users):
            if self.mode == 'unif_rand':
                ac[i] = np.random.randint(env.action_space.n)
            elif self.mode == 'rotate':
                ac[i] = ((steps + i) % env.action_space.n)
                prev_comm_ac[i] = ac[i]
            elif self.mode == 'urg5':
                ac[i] = alt_switch_action5(steps, 5)
            elif self.mode == 'heuristic':
                if i==0:
                    ac[i] = (env.heuristic_nn_action(i))
                else:
                    ac[i] = ((steps + i) % env.action_space.n)
            elif self.mode == 'best':
                if i==0:
                    ac[i], _ = env.get_best_ac(i)
                else:
                    ac[i] = ((steps + i) % env.action_space.n)
                    prev_comm_ac[i] = ac[i]
                    # ac[i] = (env.heuristic_nn_action(i))
            elif self.mode == 'bestvbest':
                ac[i], _ = env.get_best_ac(i)
        ...
        while True:
            ...
            for i in range(self.num_users):
                ...
                if self.mode == 'unif_rand':
                    ac[i] = np.array(np.random.randint(env.action_space.n))
                elif self.mode == 'rotate':
                    if ((steps + i) % self.radar_interval) == 0:        # choose radar every 4 steps
                        ac[i] = np.array(env.action_space.n - 1)
                    else:
                        if (prev_comm_ac[i] >= (env.action_space.n-3)):
                            ac[i] = 0
                        else:
                            ac[i] = prev_comm_ac[i] + 1
                        prev_comm_ac[i] = ac[i]
                elif self.mode == 'urg5':
                    ac[i] = alt_switch_action5(steps, 5)
                elif self.mode == 'heuristic':
                    if i==0:
                        ac[i] = np.array(env.heuristic_nn_action(i))
                    else:
                        ac[i] = np.array((steps + i) % env.action_space.n)
                elif self.mode == 'best':
                    if i==0:
                        ac[i], _ = env.get_best_ac(i)
                    else:
        ...
        path = {}
        path = {"reward" : np.array(rewards, dtype=np.float32),} # (steps)}
...
parser.add_argument('--mode', choices=['heuristic','urg5','best','bestvbest','csma-ca','rotate'], default='rotate')
